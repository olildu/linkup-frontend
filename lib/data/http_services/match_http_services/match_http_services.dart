...
class MatchHttpServices {
  ...
  Future<List<MatchCandidateModel>> getMatchUsers() async {
    ...
    final response = await http.get(
      Uri.parse("$BASE_URL/matches/get-matches"),
      headers: {
  ...
  Future<Map<String, dynamic>> getConnections() async {
    ...
    if (response.statusCode == 200) {
      ...
      final List<MatchesConnectionModel> matches = (jsonData["matches"] as List<dynamic>).map((json) => MatchesConnectionModel.fromJson(json)).toList();
